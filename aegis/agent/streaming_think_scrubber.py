"""Stateful scrubber for reasoning/thinking tags in streamed text."""
...
class StreamingThinkScrubber:
    """Remove reasoning blocks from text that arrives in streaming deltas."""
    ...
    def reset(self) -> None:
        """Clear all streaming state for a fresh turn."""
        self._in_block = False
        self._buf = ""
        self._last_emitted_ended_newline = True

    def feed(self, text: str) -> str:
        """Feed one delta and return the visible scrubbed text."""
        if not text:
            return ""
    ...
    def flush(self) -> str:
        """Flush held text at stream end, dropping unterminated reasoning."""
        if self._in_block:
            self._buf = ""
            self._in_block = False
            return ""
