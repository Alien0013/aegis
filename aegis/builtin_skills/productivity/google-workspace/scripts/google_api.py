#!/usr/bin/env python3
"""Google Workspace API CLI for AEGIS Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing AEGIS-facing JSON contract and falls back to the Python client
...
from _aegis_home import get_aegis_home
...
def _gws_binary() -> str | None:
    override = os.getenv("AEGIS_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for AEGIS Agent")
