#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for AEGIS Agent.
...
# Ensure sibling modules (_aegis_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _aegis_home import display_aegis_home, get_aegis_home

AEGIS_HOME = get_aegis_home()
TOKEN_PATH = AEGIS_HOME / "google_token.json"
CLIENT_SECRET_PATH = AEGIS_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = AEGIS_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same AEGIS profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the AEGIS Docker image's venv is
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to AEGIS home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_aegis_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for AEGIS")
