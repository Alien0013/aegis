...
AEGIS_CFG="${AEGIS_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure AEGIS config has twozero_td MCP entry ──
if [[ ! -f "$AEGIS_CFG" ]]; then
    echo -e " ${FAIL} AEGIS config not found at ${AEGIS_CFG}"
    manual_steps+=("Create ${AEGIS_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$AEGIS_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in AEGIS config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to AEGIS config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${AEGIS_CFG} manually"); }
    manual_steps+=("Restart AEGIS session to pick up config change")
fi
