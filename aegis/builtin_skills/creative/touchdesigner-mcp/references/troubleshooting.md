...
## 5. AEGIS Configuration

### Config location

`$AEGIS_HOME/config.yaml` (defaults to `~/.aegis/config.yaml` when `AEGIS_HOME` is unset)
...
Restart the AEGIS session for changes to take effect. The MCP connection is
