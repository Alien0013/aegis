...
_CHANNEL_CATALOG: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "capabilities": ["text", "groups", "attachments", "media", "idempotency"],
...
def create_app(config: Config) -> FastAPI:
    ...
    # Route handlers live in domain-grouped router modules under aegis/dashboard_routes/;
    # register_all wires them onto `app` in the original order (the /api and SPA catch-alls
    # register last). create_app stays a thin orchestrator.
    from . import dashboard_routes
    dashboard_routes.register_all(app, config, chat_runner)
