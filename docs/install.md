...
inline channel selection, and starter files (SOUL.md, AGENTS.md) at the `~/.aegis` root. Set
...
First-run tool and skill policy can be chosen from the installer too. The default
is cost-safe `core`; add optional toolsets only when needed:
`--toolsets core,browser --skills web-research,summarize`, or with
...
curl -fsSL https://raw.githubusercontent.com/Alien0013/aegis/main/install.sh | bash -s -- --toolsets core,browser --skills web-research,summarize
