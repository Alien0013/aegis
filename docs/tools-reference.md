...
| memory | core |  | low | a0d28f84d9db | action, content, old_text, operations, target | Save durable information to persistent memory that survives across sessions. Memory is injected into future sessions, so keep it compact and focused on facts that will still matter later. |
...
| search | core |  | low | 165e2ead7502 | context, file_glob, glob, limit, offset, output_mode, path, pattern, target | Search file contents or find files by name. Supports reference-style search_files pagination and output modes. |
| search_files | core |  | low | 098fa1083445 | context, file_glob, glob, limit, offset, output_mode, path, pattern, target | Compatibility alias for `search`. |
...
| skill_manage | core |  | low | 233603063259 | action, body, content, description, file_content, file_path, into, name, new_string, old_string, overwrite, replace_all | Manage skills with an action-style schema. Actions: list, view, create, edit, patch, write_file, remove_file, delete, usage, pin, unpin, report, consolidate. Uses AEGIS SkillsLoader for discovery/create/view/usage and the curator for pinning, reports, recoverable delete-by-archive, and consolidating overlapping skills into one. |
...
| tool_call | core |  | low | 328e8027e19e | arguments, name | Invoke a deferred tool by name with arguments matching its schema. |
| tool_describe | core |  | low | 8c48af751d2d | name | Load the full parameter schema for one deferred tool returned by tool_search. |
