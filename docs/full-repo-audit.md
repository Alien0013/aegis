...
1. Desktop lifecycle should be formalized as a state machine: missing, starting, ready, degraded, crashed, updating.
2. Renderer connection state should be durable and visible.
3. Desktop repair/checklist page should guide missing backend/config/provider fixes.
4. Packaged smoke test should launch the built artifact and verify `/api/health` with token.
5. Release signing/notarization is guarded by tests but still needs CI artifact proof.

Verdict: desktop is more mature than expected; next work is lifecycle observability and packaged smoke coverage.
...
- Security scanner and permission engine are strong for a local-first agent, but dashboard explanation of policy decisions is missing.

Gaps:

1. Security audit should output JSON and Markdown reports.
...
3. Add a release gate command that runs Python, web, desktop, docs, installer, package, and security checks.
4. Add SBOM/release artifact hashes.
5. Add installer/update rollback coverage.

Verdict: security model is strong; release provenance and UI visibility are the weak spots.
...
Verdict: docs are decent but product claims/roadmap have drift.

---

## 18. Highest-Priority Build Plan

To move fastest toward full production feature parity, land these as TDD slices, not one risky rewrite.

### P0 — Keep CI green and protect workspace

1. Commit or stage current verified lint/test fixes.
2. Update README test count after final CI confirms.
3. Rewrite stale roadmap sections to match current scope.
4. Add `docs/full-repo-audit.md` and `docs/production-harness-plan.md` to docs index.

### P1 — Tool schema + permission control plane

1. Add all-tool schema validator test.
2. Add `PermissionEngine.explain()` returning structured allow/deny reasons.
3. Add dashboard `/api/tools/permission-dry-run`.
4. Add Tools page dry-run UI.

### P2 — Provider capability matrix

1. Add provider capability fields.
2. Add provider contract tests for transport modes.
3. Add dashboard auth/probe status.
4. Add trace spans for fallback/recovery reasons.

### P3 — Trace/run/session explainability

1. Add trace timeline endpoint.
2. Add dashboard trace timeline page.
3. Add session lineage graph.
4. Extend eval diff to tools/cost/latency/errors.

### P4 — Gateway/cron ops reliability

1. Add delivery dead-letter store.
2. Add queue/backpressure metrics.
3. Add cron dry-run/next-fire preview.
4. Add gateway operations dashboard.

### P5 — Desktop production state machine

1. Formal backend lifecycle enum.
2. Renderer connection store.
3. Packaged desktop smoke test.
4. Desktop repair checklist.

### P6 — Memory/skills governance

1. Skill quality gate for install/create/edit.
2. Skill usage telemetry page.
3. Memory provenance dashboard.
4. Curator dry-run/approval workflow.

### P7 — Release gate and provenance

1. Add `scripts/verify_release.py`.
2. Include Python/web/desktop/site/security/doc checks.
3. Generate release manifest with hashes.
4. Add update rollback test.
...
1. explainability and control-plane UI,
2. provider/tool contract hardening,
3. gateway/cron operational visibility,
4. release provenance,
5. stale docs/roadmap cleanup,
6. modularization of the largest runtime/API files after tests pin behavior.

Recommended next coding slice: **tool schema validator + permission dry-run endpoint**, because it improves every future surface and catches broken production extensions early.
